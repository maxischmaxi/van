...
static cJSON *build_mcp_server_json(McpServer *s) {
    cJSON *obj = cJSON_CreateObject();
    cJSON_AddStringToObject(obj, "serverName", s->server_name ? s->server_name : "");
    cJSON_AddStringToObject(obj, "serverUrl", s->server_url ? s->server_url : "");
    cJSON_AddStringToObject(obj, "accessToken", s->access_token ? s->access_token : "");
    cJSON_AddItemToObject(obj, "discoveryState", build_discovery_json(&s->discovery_state));
    cJSON_AddStringToObject(obj, "clientId", s->client_id ? s->client_id : "");
    cJSON_AddStringToObject(obj, "redirectUri", s->redirect_uri ? s->redirect_uri : "");
    return obj;
}
...
static cJSON *build_mcp_oauth_json(McpOAuth *m) {
    ...
    for (size_t i = 0; i < m->count; i++) {
        ...
        cJSON_AddItemToObject(obj, m->entries[i].key ? m->entries[i].key : "", server);
    }
    return obj;
}
...
static cJSON *build_claude_ai_oauth_json(ClaudeAiOauth *a) {
    cJSON *obj = cJSON_CreateObject();
    cJSON_AddStringToObject(obj, "accessToken", a->accessToken ? a->accessToken : "");
    cJSON_AddStringToObject(obj, "refreshToken", a->refreshToken ? a->refreshToken : "");
    cJSON_AddNumberToObject(obj, "expiresAt", (double)a->expiresAt);
    cJSON_AddNumberToObject(obj, "refreshTokenExpiresAt", (double)a->refreshTokenExpiresAt);

    cJSON *scopes = cJSON_CreateArray();
    for (size_t i = 0; i < a->scopes_count; i++) {
        cJSON_AddItemToArray(scopes, cJSON_CreateString(a->scopes[i] ? a->scopes[i] : ""));
    }
    cJSON_AddItemToObject(obj, "scopes", scopes);

    cJSON_AddStringToObject(obj, "subscriptionType", a->subscriptionType ? a->subscriptionType : "");
    cJSON_AddStringToObject(obj, "rateLimitTier", a->rateLimitTier ? a->rateLimitTier : "");
    return obj;
}
